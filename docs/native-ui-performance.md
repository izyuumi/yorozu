...
Regression coverage: `cachedSyncPageDoesNotBlockTheMainActorForAFrameBurst`, `backgroundThreadEventsDoNotInvalidateTheOpenThreadsTimeline`, `unchangedTimelineReusesGroupingAndInvalidatesForResultsAndRunningState`, existing burst/paced streaming tests, and the keyboard UI harness. Sync-page timing starts at the first applied event, excluding scheduler delay from unrelated concurrent tests. The outbox retry test checks stable IDs rather than exactly-once transport delivery: reconnect may resend before receipt, and runtime deduplication is the contract.
