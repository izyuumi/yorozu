...
- **Connect your devices** — `PairingSheet` embedded (`embedded: true` drops its own title,
  ...
  polling.
- **Permissions** — existing `PermissionsView(scope: .onboarding)` grouped Form, which handles its
  own scrolling. Everything optional; the settings design is reused unchanged.
...
Isolated commit checked on 2026-09-25 against GitHub `main` (`e5b695e`):

- Mac build/tests: 10 passed after integrating the latest settings changes. iOS Simulator build passed.
- Shared Swift: 274 passed. An existing reconnect test was corrected to wait for its
...
- Latest settings changes remain intact. Permissions reuse the existing grouped Form,
  and the Devices settings Form is unchanged. No shared/runtime production edits were added.
