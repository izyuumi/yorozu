...
// MARK: - What a Live Activity is told

private func event(_ payload: YorozuEvent.Payload, agent: String = "main", parent: String? = nil)
    -> YorozuEvent
{
    YorozuEvent(id: UUID().uuidString, threadId: "t", ts: 0, agentId: agent, parentAgentId: parent, payload: payload)
}

@Test func aTurnRunsUntilTheMainAgentIsDone() {
    let working = event(.message(MessageData(role: .agent, text: "…")))
    #expect(TurnProgress.status(after: working, device: "phone") == .working)

    // A delegated agent finishing is one step of the turn, not the end of it.
    let delegate = event(.message(MessageData(role: .agent, text: "done", done: true)), agent: "sub", parent: "main")
    #expect(TurnProgress.status(after: delegate, device: "phone") == .working)

    let done = event(.message(MessageData(role: .agent, text: "there", done: true)))
    #expect(TurnProgress.status(after: done, device: "phone") == .done)
}

@Test func aCardParksTheTurnOnAPerson() {
    let card = event(.approvalCard(ApprovalCardData(actionId: "a", actionClass: "run-command", target: "rm -rf /")))
    #expect(TurnProgress.status(after: card, device: "phone") == .needsApproval)
    #expect(TurnProgress.status(after: card, device: "phone")?.isLive == true)

    let answered = event(.approvalAnswer(ApprovalAnswerData(actionId: "a", answer: .yes)))
    #expect(TurnProgress.status(after: answered, device: "phone") == .working)
}

@Test func stoppingATurnEndsIt() {
    #expect(TurnProgress.status(after: event(.interrupt(InterruptData())), device: "phone") == .failed)
    #expect(TurnStatus.failed.isLive == false)
}

@Test func onlyOurOwnMessageStartsATurnWeAreWatching() {
    let ours = event(.message(MessageData(role: .user, text: "hi")), agent: "phone")
    #expect(TurnProgress.status(after: ours, device: "phone") == .working)

    // The Mac's own composer, or another phone: their turn, and not one this device started.
    let theirs = event(.message(MessageData(role: .user, text: "hi")), agent: "mac")
    #expect(TurnProgress.status(after: theirs, device: "phone") == nil)
}

@Test func housekeepingSaysNothingAboutATurn() {
    #expect(TurnProgress.status(after: event(.threadList(ThreadListData(threads: []))), device: "phone") == nil)
    #expect(TurnProgress.status(after: event(.threadRename(ThreadRenameData(title: "x"))), device: "phone") == nil)
}
